...
import numpy as np
import pdb
# 박동우, 문지원, 정현석, 김영헌, 황성수 (2018). 속도와 시야각에 강인한 다중 ArUco 마커 기반 증강현실 시스템. 한국HCI학회 학술대회, 916-920 <그림2> 참조
...
#        print(rect)
   ...
   #     print('ch', r, r[2][0])
            if rot_id == 0:
             ...
             #   cv2.putText(frame, str('1'), (int(c1[0]), int(c1[1])), cv2.FONT_HERSHEY_SIMPLEX, 0.6,(0, 255, 0), 2, cv2.LINE_8)
             #   cv2.putText(frame, str('4'), (int(c4[0]), int(c4[1])), cv2.FONT_HERSHEY_SIMPLEX, 0.6,(0, 255, 0), 2, cv2.LINE_8)
             #   cv2.putText(frame, str('3'), (int(c3[0]), int(c3[1])), cv2.FONT_HERSHEY_SIMPLEX, 0.6,(0, 255, 0), 2, cv2.LINE_8)
             #   cv2.putText(frame, str('2'), (int(c2[0]), int(c2[1])), cv2.FONT_HERSHEY_SIMPLEX, 0.6,(0, 255, 0), 2, cv2.LINE_8)
...
#print(aruco_dict_bytes.shape)
#print(aruco_dict_bytes[1].reshape(4,5))
...
#print(camMat['mat'], camMat['rvecs'], camMat['tvecs'])
...
out = cv2.VideoWriter('output.mp4', fourcc, fps/3, (w, h))
